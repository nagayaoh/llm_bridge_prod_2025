...
srun --nodes=1 --ntasks=1 -w "$head_node" \
  bash -c "unset ROCR_VISIBLE_DEVICES; \
           source "$(conda info --base)/etc/profile.d/conda.sh" && conda activate llmbench && \
...
srun --overlap -N1 -n1 -c1 --gpus=0 -w "$head_node" \
  bash -c "/home/Competition2025/adm/X005/.conda/envs/llmbench/bin/python - <<'PY'
import ray, json
